...
public class MemoryInfo {
	...
	public String getTotalMemory() {
		...
		return new DecimalFormat().format((double) memory / 1024);
	}
	...
	public int getPidMemorySize(int[] pids, Context context) {

		int memSize = 0;
		ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
		Debug.MemoryInfo[] memoryInfo = am.getProcessMemoryInfo(pids);
		for(int j = 0; j < memoryInfo.length; j++){
			memoryInfo[j].getTotalSharedDirty();
			memSize += memoryInfo[j].getTotalPss();
		}
		return memSize;
	}
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (line.contains("Permission Denial")) {
					...
					if (line.contains("MEMINFO in pid")) {
						...
						String[] lineItems = line.split("\\s+");//把字符串分割成数组
